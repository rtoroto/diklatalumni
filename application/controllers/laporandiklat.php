<?php
class laporandiklat extends CI_Controller
{
    
    var $folder =   "laporandiklat";
    var $tables =   "";
    var $pk     =   "";
    var $title  =   "Laporan Diklat";
...
//        }
        ...
        $this->template->load('template', 'laporandiklat/view',$data);
    }
...
//        $data['tahun_angkatan']=  $this->db->get('akademik_tahun_akademik')->result();
        $this->template->load('template', 'laporandiklat/laporan',$data);
    }
    
    function cetak()
    {
        $jenis="SELECT * FROM diklat";
...
//        $data['tahun']=  $this->db->get('student_angkatan')->result();
        ...
        $this->load->view('laporandiklat/cetak',$data);
    }
    ...
    function tampilkandata($konsentrasi,$tahun_akademik)
    {
        // fires else
            ...
            echo"<table class='table table-bordered'>
                <tr class='success'>
                <th colspan=".($smt_aktif+4).">PEMBAYARAN SOT | BIAYA SOT PERSEMESTER = RP. ". rp((int) $spp_bayar) ."</th>
                </tr>
                <tr>
                <th width='10'>No</th>
                <th width='60'>NIM</th>
